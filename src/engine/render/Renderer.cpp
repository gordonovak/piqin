...
void Renderer::render(std::vector<Gear*>& gears) {
	...
	// This will be scaled up after we draw the background & actors
	set_render_texture();
	// Draw the background
	draw_background();
...
void Renderer::present() {
	...
	SDL_Rect fr = {0, 0, static_cast<int>(global::scene.width * scale), static_cast<int>(global::scene.height * scale)};
	// These next two lines center the canvas.
	fr.x = static_cast<int>((canvasWidth-(global::scene.width*scale))/2.0);
	fr.y = static_cast<int>((canvasHeight-(global::scene.height*scale))/2.0);
...
void Renderer::update_canvas_size(){
	...
	if (w != canvasWidth || h != canvasHeight) {
		...
		scale = ((1.0 * canvasHeight) / canvasWidth < 0.75) ? 1.0*canvasHeight/global::scene.height : 1.0*canvasWidth/global::scene.width;
